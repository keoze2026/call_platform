...
class Command(BaseCommand):
    help = "Backfills missing CallRecords from CallLog for Asterisk routed calls"

    def handle(self, *args, **options):
        self.stdout.write("Starting backfill sync...")
        count = 0
        total_checked = 0
        logs = CallLog.objects.exclude(status__in=['ringing', 'in_progress'])
        ...
        self.stdout.write(self.style.SUCCESS(f"Sync complete. Checked {total_checked} calls, successfully created {count} missing CallRecords."))
